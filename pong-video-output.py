...
"""
Created on Sun May 10 16:20:01 2020
...
def plot_results():
    ...
    plt.title('Rewards')
    plt.plot(rewards_total, alpha = 0.8, color = 'red')
...
class QNetAgent(object):
    ...
    def optimize(self):
        ...
        state = [process_frame(frame) for frame in state]
        state = torch.cat(state)
        
        new_state = [process_frame(frame) for frame in new_state]
...
print('Average rewards: %.2f' % (sum(rewards_total)/num_episodes)) 
print('Average rewards for last 100 steps: %.2f' % (sum(rewards_total[-100:])/100))
if solved:
...
plt.figure(figsize=(12,5))
plt.title('Rewards')
plt.bar(torch.arange(len(rewards_total)), rewards_total, alpha = 0.8, color = 'red')
plt.show()
